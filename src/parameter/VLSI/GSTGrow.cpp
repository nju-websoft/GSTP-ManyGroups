...
int solve2(vector< pair<int,int> > P, vector< pair< pair<int,int>,pair<int,int> > > &Ans, pair<int,int> rt)
{
	...
	int ansW=0;
	for(int i=1;i<=dx;++i)
		for(int j=1;j<=dy;++j)dis_table[i][j]=inf;
	...
	for(int i=0;i<P.size();++i)if(!inT[P[i].first][P[i].second])
	{
		pair<int,int> v=P[i];
		pair<int,int> u=u_table[v.first][v.second];
		...
		for(auto x:P)if(!inT[x.first][x.second])
		{
			int nd=getD(v,x);
			if(nd<dis_table[x.first][x.second])
			{
				dis_table[x.first][x.second]=nd;
				u_table[x.first][x.second]=v;
			}
		}
	}
	return ansW;
